package linear_data_structure.List.singleLinked;


import linear_data_structure.List.ILinkedList;

/**
 * Created by zejian on 2016/10/22.
 * 带头结点并含有尾指针的链表
 * 链式存储方式对逻辑上相邻的结点不要求在存储的物理位置上也相邻，结点间的逻辑关系是由附加的”指针“来表示的，
 * 所以存储上，一个结点包含两部分内容，一部分是当前结点的数据内容，一部分是当前结点下一个结点的存储地址（指针）
 */
